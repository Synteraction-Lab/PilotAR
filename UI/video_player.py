...
from UI.widget_generator import get_bordered_frame
...
class Player(Tk.Frame):
    ...
    def __init__(self, parent, video=''):
        ...
        # fileMenu.add_shortcut("Play", 'p', self.play)  # Play/Pause
        # fileMenu.add_command(label="Stop", command=self.stop)
        # fileMenu.add_separator()
        ...
        self.videopanel = get_bordered_frame(self.parent)
        self.canvas = Tk.Canvas(self.videopanel)
        self.canvas.pack(fill=Tk.BOTH, expand=1)
        self.videopanel.pack(fill=Tk.BOTH, expand=1)

        # panel to hold buttons
        self.buttons_panel = ttk.Frame(self.parent)
        self.buttons_panel.pack(side="left")
        ...
        # stop = ttk.Button(buttons, text="Stop", command=self.stop)
        ...
        self.volSlider = ttk.Scale(buttons, variable=self.volVar, command=self.OnVolume,
        ...
        # self.timeSlider = ttk.Scale(timers, variable=self.timeVar, command=self.seek,
        ...
        if _isMacOS:
            ...
            self.buttons_panel.bind("<Button-1>",
                                    lambda event: setattr(self, "has_clicked_on_buttons_panel", event.y < 0))
    ...
    def _Play(self, video):
        # helper for OnOpen and play
        if not isfile(self.video):  # Creation
            return
    ...
    def pause(self, *unused):
        ...
        if self.player.get_media():
            ...
            # self.player.stop()  # toggles

    def video_info(self):
        return {"duration": int(self.player.get_length() * 1e-3)}

    def current_duration(self):
        return self.player.get_time() * 1e-3
    ...
    def OnTick(self):
        ...
        if self.player:
            ...
            if t > 0:
                ...
                if t > 0 and time.time() > (self.timeSliderUpdate + 0.1):
                    # self.timeSlider.set(t)
                    self.event_generate("<<SecondChanged>>")
                    self.timeSliderLast = int(self.timeVar.get())
            if abs(self.player.get_time() * 1e-3 - (self.player.get_length() * 1e-3)) < 0.5 \
                    and self.player.get_length() > 0:
                self.stop()
            # print(float(self.player.get_time() * 1e-3), float(self.player.get_length() * 1e-3),
            #       self.player.is_playing())
        # start the 0.5 second timer again
        ...
        if not self._geometry and _isMacOS:
            self.OnResize()
    ...
    def screenshot_current_frame(self, save_location_path):
